...
- Every "finding" must cite specific numbers and actual values from the crawl data
  GOOD: "Title tag is 98 characters — 38 chars over the 60-char limit. Current: 'Equipment Hire & Sales Australia | Supply Nation Certified | Access Indigenous - Access Indigenous'"
  BAD:  "Title tag is too long"
- Every "recommendation" must be specific, step-by-step, and actionable for THIS site
  GOOD: "Shorten the title to under 60 chars. Suggested: 'Equipment Hire & Sales | Access Indigenous' — leads with the service, includes the brand name, targets 'equipment hire australia'"
  BAD:  "Shorten the title tag"
- For image issues: state the exact count (e.g. '9 of 20 images') and describe what types of images are affected (hero images, product images, logos, etc.)
...
def generate_report(analysis: dict, url: str, output_path: str, previous_run: dict = None, crawl_data: dict = None):
    ...
    # ── APPENDIX: RAW CRAWL DATA (Python-generated, no AI) ──
    if crawl_data:
        doc.add_page_break()
        doc.add_heading("Appendix: Raw Crawl Data", level=1)
        add_styled_paragraph(doc, "All data below was collected directly from the live page crawl. This section is generated by Python — not AI — and contains the exact values used during analysis.", 10, False, GRAY)

        # A1. Page Meta
        doc.add_heading("A1. Page Meta Data", level=2)
        meta_rows = [
            ("Title Tag",         f"{crawl_data.get('title') or 'NOT SET'} ({crawl_data.get('title_length', 0)} chars — optimal 50–60)"),
            ("Meta Description",  f"{crawl_data.get('meta_description') or 'NOT SET'} ({crawl_data.get('meta_description_length', 0)} chars — optimal 120–160)"),
            ("Canonical URL",     crawl_data.get('canonical_url') or 'Not set'),
            ("HTML Lang",         crawl_data.get('html_lang') or 'NOT SET'),
            ("Viewport",          'Set' if crawl_data.get('has_viewport') else 'NOT SET'),
            ("Meta Robots",       crawl_data.get('meta_robots') or 'Not set (defaults to index, follow)'),
            ("Charset",           crawl_data.get('charset') or 'Not set'),
            ("HTTPS",             'Yes' if crawl_data.get('is_https') else 'No'),
            ("Response Time",     f"{crawl_data.get('response_time_ms', 0)} ms"),
            ("Page Size",         f"{crawl_data.get('content_length', 0):,} bytes"),
            ("Word Count",        str(crawl_data.get('word_count', 0))),
            ("Paragraphs",        str(crawl_data.get('paragraph_count', 0))),
            ("Internal Links",    str(crawl_data.get('internal_link_count', 0))),
            ("External Links",    str(crawl_data.get('external_link_count', 0))),
            ("Phone Links",       ', '.join(crawl_data.get('tel_links', [])) or 'None found'),
            ("Schema Types",      ', '.join(crawl_data.get('schema_types', [])) or 'None detected'),
            ("Tech Detected",     ', '.join(crawl_data.get('tech_detected', [])) or 'None'),
            ("Forms Found",       str(crawl_data.get('form_count', 0))),
            ("Mixed Content",     f"{len(crawl_data.get('mixed_content', []))} items" if crawl_data.get('mixed_content') else 'None'),
        ]
        t = doc.add_table(rows=1, cols=2)
        style_header_cell(t.rows[0].cells[0], "Property")
        style_header_cell(t.rows[0].cells[1], "Value")
        for label, value in meta_rows:
            r = t.add_row().cells
            style_cell(r[0], label, bold=True)
            style_cell(r[1], value)

        # A2. Heading Structure
        doc.add_heading("A2. Heading Structure", level=2)
        headings = crawl_data.get('headings', {})
        has_headings = any(v for v in headings.values())
        if has_headings:
            t = doc.add_table(rows=1, cols=2)
            style_header_cell(t.rows[0].cells[0], "Level")
            style_header_cell(t.rows[0].cells[1], "Text")
            for level in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
                for text in headings.get(level, []):
                    r = t.add_row().cells
                    style_cell(r[0], level.upper(), bold=True)
                    style_cell(r[1], text)
        else:
            add_styled_paragraph(doc, "No headings found.", 10, False, GRAY)

        # A3. Image Audit
        doc.add_heading("A3. Image Audit (All Images)", level=2)
        images = crawl_data.get('images', [])
        total_imgs = crawl_data.get('image_count', len(images))
        missing_alt = sum(1 for img in images if not img.get('has_alt'))
        no_dims = sum(1 for img in images if not img.get('has_dimensions'))
        no_lazy = sum(1 for img in images if not img.get('has_lazy_loading'))
        add_styled_paragraph(doc,
            f"Total: {total_imgs} images — "
            f"Missing alt text: {missing_alt} — "
            f"Not lazy-loaded: {no_lazy} — "
            f"Missing dimensions: {no_dims}",
            10, True, DARK)
        if images:
        ...
        # A4. External Links
        doc.add_heading("A4. External Links", level=2)
        ext_links = crawl_data.get('external_links', [])
        if ext_links:
            t = doc.add_table(rows=1, cols=2)
            style_header_cell(t.rows[0].cells[0], "Anchor Text")
            style_header_cell(t.rows[0].cells[1], "URL")
            for link in ext_links:
                r = t.add_row().cells
                style_cell(r[0], link.get('text', '') or '(no text)')
                style_cell(r[1], link.get('href', '')[:120])
        else:
            add_styled_paragraph(doc, "No external links found.", 10, False, GRAY)

        # A5. Open Graph & Twitter Tags
        doc.add_heading("A5. Open Graph & Social Tags", level=2)
        og = crawl_data.get('og_tags', {})
        tw = crawl_data.get('twitter_tags', {})
        if og or tw:
            t = doc.add_table(rows=1, cols=2)
            style_header_cell(t.rows[0].cells[0], "Tag")
            style_header_cell(t.rows[0].cells[1], "Value")
            for tag, val in {**og, **tw}.items():
                r = t.add_row().cells
                style_cell(r[0], tag, bold=True)
                style_cell(r[1], str(val)[:150])
        else:
            add_styled_paragraph(doc, "No OG or Twitter tags found.", 10, False, GRAY)

        # A6. Schema / Structured Data
        doc.add_heading("A6. Structured Data (JSON-LD)", level=2)
        schema_types = crawl_data.get('schema_types', [])
        if schema_types:
            for s in schema_types:
                add_styled_paragraph(doc, f"• {s}", 10, False, DARK)
        else:
            add_styled_paragraph(doc, "No JSON-LD schema detected.", 10, False, GRAY)
    ...
    add_styled_paragraph(doc, "Appendix data is Python-generated from raw crawl results. AI analysis sections reference this data for findings and recommendations.", 10, False, GRAY)
...
def main():
    ...
    generate_report(analysis, url, output_path, previous_run=previous_run, crawl_data=crawl_data)
